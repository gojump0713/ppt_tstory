...
(async () => {
  ...
  await page.goto(URL + "#slide-17", { waitUntil: "load" }); await sleep(3500);
  await page.evaluate(() => { const bg = document.querySelector("#sec-17 .bg"); bg.classList.add("has-media"); const d = document.createElement("div"); d.className = "media ready"; d.style.cssText = "position:absolute;inset:0;background:linear-gradient(135deg,#5a4a3a,#3a3a2a,#1e2a44)"; bg.insertBefore(d, bg.querySelector(".veil")); });
  await sleep(1200); await page.screenshot({ path: path.join(OUT, "hero-media-17.png") });
  // 2) slide 12 after .ask fix
  await page.goto(URL + "#slide-12", { waitUntil: "load" }); await sleep(5000); await page.screenshot({ path: path.join(OUT, "s13.png") });
  ...
  // 6) slide 11: drag then ArrowRight should move to slide 12
  await page.goto(URL + "#slide-11", { waitUntil: "load" }); await sleep(4000);
  ...
  await page.goto(URL + "#slide-16", { waitUntil: "load" }); await sleep(1500);
